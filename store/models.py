...
class Product(models.Model):
    ...
    last_update = models.DateTimeField(
        auto_now=True)  # auto_now every time we update the product object this property sets time to that date/time
    #                                                     auto_now_add - with this only time when we create Product will be stored
    collection = models.ForeignKey('Collection', on_delete=models.PROTECT)
    promotions = models.ManyToManyField(
        'Promotion')  # related_names='products' we can add this if we want to make custom name for field
